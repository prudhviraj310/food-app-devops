pipeline {
    ...
    environment {
        // Private IP is correct for internal communication
    ...
    stages {
        ...
        stage('Security Scan (Trivy)') {
            steps {
                ...
                sh "docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v ${WORKSPACE}:/root ghcr.io/aquasecurity/trivy:0.69.3 fs /root"
            }
        }
        ...
        stage('Static Analysis (SonarQube)') {
            steps {
                ...
                withCredentials([string(credentialsId: 'sonar-token', variable: 'SONAR_TOKEN')]) {
                    sh """
                    # This is the critical fix: Ensure the folder is readable by everyone
